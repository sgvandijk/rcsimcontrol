...
#ifndef SC_PREDICATE_HH_
#define SC_BATS_PREDICATE_HH_
...
namespace sc {
  ...
  public:
    ...
    static bool pred_comp(PredicatePtr p, std::string const &val)
    {
      return p->getStr() < val;
    }
  ...
  public:
    ...
    */
    ...
    {
      PredicatePtr pred(new Predicate(str,type_leaf));
      push(pred);
    }
    ...
    void pushLeaf(unsigned val)
    {
      std::ostringstream s;
      s << val;
      PredicatePtr pred(new Predicate(s.str(),type_leaf));
      push(pred);
    }
    ...
    {
      PredicatePtr pred(new Predicate(key));
      pred->pushLeaf(value);
      push(pred);
    }
