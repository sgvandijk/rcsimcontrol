...
namespace sc {
  ...
  public:
    ...
    public:
       ...
       */
      NodePtr getChild(unsigned index)
      {
       ...
       *  @returns the child at index.
       */
      NodePtr const getChild(unsigned index) const
      {
        assert(index < d_nodes.size());
        return d_nodes[index];
      }
      ...
      NodePtr operator[](unsigned index) { return getChild(index); }

      /**
       *  Adds a child node to this node.
       */
      NodePtr addChild(NodePtr child) { d_nodes.push_back(child); return child; }
       ...
       */
      NodePtr find(std::string const &query) const;

      /**
       *  Finds all nodes witch match query and appends them to res.
       *  @returns the number of matches.
       */
      unsigned findAll(NodeVector &res, std::string const &query) const;
       ...
       *
       */
      NodePtr findDeep(std::string const &query) const;
       ...
       */
      unsigned findAllDeep(NodeVector &res, std::string const &query) const;
       ...
       */
      unsigned selectAll(NodeVector &res, Path const &select) const;

      /**
       *  @returns the first predicate that complies with select.
       */
      NodePtr select(Path const &select) const;
